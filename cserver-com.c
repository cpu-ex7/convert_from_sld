 ...
 */
static void send_sld_data(char* filename)
{
  fprintf(stderr, "sending SLD data:\n");
//  startTime = GetTickCount();
  gettimeofday(&startTime, NULL);
  com_write((char*)sld_words, sld_n_words*sizeof(sld_words[0]), filename);
...
//  size_t elapsedTime = endTime - startTime;
  ...
  fprintf(stderr, "elapsed time : %lf s\n",
		  ...
		  (double)(endTime.tv_usec - startTime.tv_usec)/(1000*1000));
  fclose(out);
  */
